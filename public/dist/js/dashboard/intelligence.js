...
function renderRiskModuleSummary(moduleData = {}) {
  ...
  if (!summary.total_assessed_responses) {
    return '<div class="text-muted">Sem respostas avaliadas neste periodo.</div>';
  }
  return [
...
async function loadDashboard() {
  ...
  document.getElementById('metrics-row').innerHTML = [
    ...
    metricCard('Usuarios unicos', metrics.unique_requesters || 0, 'secondary'),
    metricCard('Resolucao automatica', (metrics.automatic_resolution_rate || 0) + '%', 'success'),
    ...
    metricCard('Tempo medio', (metrics.avg_response_time_seconds || 0) + 's', 'primary'),
    metricCard('Pendentes de revisao', metrics.pending_reviews || 0, 'danger')
  ].join('');
