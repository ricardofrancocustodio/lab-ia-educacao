-- Registrar pagina "notices" e permissoes padroes por perfil
...
values ('notices', 'Mural de Comunicados', 25, true)
on conflict (key) do update
set label = excluded.label,
    menu_order = excluded.menu_order,
    active = excluded.active;

-- 2. Atualizar a funcao seed para incluir a nova pagina
...
begin
  ...
  r := 'network_manager';
  foreach p in array array['dashboard','chat-manager','reports','audit','users','preferences','knowledge','official-content','notices'] loop
  ...
  r := 'content_curator';
  foreach p in array array['dashboard','reports','audit','knowledge','official-content','notices'] loop
    insert into public.role_page_permissions (school_id, role, page_key, allowed)
    values (p_school_id, r, p, true)
    on conflict (school_id, role, page_key) do update set allowed = excluded.allowed, updated_at = now();
  end loop;
  ...
  r := 'public_operator';
  foreach p in array array['dashboard','chat-manager','reports','knowledge','official-content','notices'] loop
    insert into public.role_page_permissions (school_id, role, page_key, allowed)
    values (p_school_id, r, p, true)
    on conflict (school_id, role, page_key) do update set allowed = excluded.allowed, updated_at = now();
  end loop;
  ...
  r := 'coordination';
  foreach p in array array['dashboard','chat-manager','reports','knowledge','notices'] loop
    insert into public.role_page_permissions (school_id, role, page_key, allowed)
    values (p_school_id, r, p, true)
    on conflict (school_id, role, page_key) do update set allowed = excluded.allowed, updated_at = now();
  end loop;
